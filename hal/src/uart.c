...
void uart_read(int uartFileDescriptor, uint8_t* rx_buffer, const uint MAX_SIZE)
{
    // Clear buffer before read
    memset(rx_buffer, 0, MAX_SIZE * sizeof(*rx_buffer));
    int bytesRead = read(uartFileDescriptor, rx_buffer, sizeof(rx_buffer));
    if (bytesRead < 0) {
